...
export default function Chatbot() {
  ...
  const [charactersPerTick, setCharactersPerTick] = useState(5);
  ...
  const khangInfo = {
    ...
    skills: {
      ...
      databaseSystems: ["MongoDB", "AWS", "Supabase", "PostgreSQL"],
      officeProductivity: [
    ...
    projects: [
      ...
      },
      {
        ...
        role: "Full-Stack Developer",
        technologies:
          "Python, Django, Supabase, Boostrap, RESTs API, TypeScript",
        description: [
  ...
  useEffect(() => {
    ...
    const typingInterval = setInterval(() => {
      ...
      if (currentIndex < maxIndex) {
        // Process multiple characters per tick for much faster typing
        const nextIndex = Math.min(currentIndex + charactersPerTick, maxIndex);
    ...
    async function testGeminiAPI() {
      try {
        ...
        const result = await model.generateContent(
          "Hello, can you respond with a simple 'Hello world'?"
        );
  ...
  const components = {
    // Custom link component to open links in new tab with proper styling
    a: ({ node, ...props }) => (
      <a
        {...props}
        target="_blank"
  ...
  // Function to process and enhance links in the text
  const enhanceLinks = (text) => {
    // First pass: convert plain URLs to markdown links
    ...
    let processedText = text.replace(urlRegex, (url) => {
      ...
      try {
        ...
        // If URL parsing fails, use the original URL as display text
      }
      return `[${displayText}](${url})`;
    });
    
    // Second pass: clean up GitHub links specifically (a common duplicate issue)
    // Convert [github.com/user/repo] text followed by URL to just one link
    processedText = processedText.replace(
      /\[github\.com\/([^\]]+)\](?!\()[\s]*(?:\(|<)(https:\/\/github\.com\/\1)(?:\)|>)/gi,
      '[github.com/$1]($2)'
    );
    
    // Remove duplicate GitHub links that appear in brackets and parentheses
    processedText = processedText.replace(
      /\[(github\.com\/[^)]+)\](?:\([^)]+\))[\s]*\1/gi,
      '[$1](https://$1)'
    );
  ...
  // Process message text before displaying - Remove duplicate links and standardize format
  const processMessageText = (text) => {
    // First, convert plain URLs to markdown links
    const textWithEnhancedLinks = enhanceLinks(text);
    
    // Then handle duplicate link patterns - a common issue in Gemini responses
    // This regex looks for repeated links to the same URL with different formatting
    const processedText = textWithEnhancedLinks.replace(
      /(\[[^\]]+\]\([^)]+\))\s*(\[[^\]]+\]\(\1\)|\(\1\)|\[\1\]|\1)/g, 
      '$1'
    );
    
    // Remove cases where URL is repeated in different formats (common with GitHub links)
    return processedText.replace(
      /\[(github\.com\/[^)]+)\]\((https:\/\/github\.com\/[^)]+)\)(\s*\[[^\]]+\]\((https:\/\/github\.com\/[^)]+)\)|\s*\(https:\/\/github\.com\/[^)]+\)|\s*https:\/\/github\.com\/[^)\s]+)/gi,
      '[$1]($2)'
    );
  };
        ...
        <div className="bg-white rounded-lg shadow-xl w-96 sm:w-80 md:w-96 overflow-hidden border border-gray-200">
          ...
          {!isMinimized && (
            <>
              {/* Chat messages area */}
              <div
                className="h-96 overflow-y-auto p-4 flex flex-col gap-3 bg-gray-50"
                id="chat-messages"
              >
