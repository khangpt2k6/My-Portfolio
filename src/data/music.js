/**
 * Music Data
 * -------------------------------------------------------
 * Local mp3 files from /public/music/
 * image: album art path (null = use gradient fallback)
 */

const tracks = [
  {
    id: 1,
    title: "Perfect",
    artist: "Ed Sheeran",
    src: "/music/Ed Sheeran - Perfect.mp3",
    image: "/Ed_Sheeran_Perfect_Single_cover.jpg",
    gradient: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
  },
  {
    id: 2,
    title: "Closer",
    artist: "The Chainsmokers ft. Halsey",
    src: "/music/The Chainsmokers - Closer (Lyric) ft. Halsey.mp3",
    image: "/closer.jpg",
    gradient: "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)",
  },
  {
    id: 3,
    title: "River Flows in You",
    artist: "Yiruma",
    src: "/music/Yiruma - River Flows in You.mp3",
    image: "/river flow in you.jpg",
    gradient: "linear-gradient(135deg, #89f7fe 0%, #66a6ff 100%)",
  },
  {
    id: 4,
    title: "Someone Like You",
    artist: "Adele",
    src: "/music/Someone Like You - Adele (Lyrics).mp3",
    image: "/someone like you.jpg",
    gradient: "linear-gradient(135deg, #c3cfe2 0%, #667eea 50%, #764ba2 100%)",
  },
  {
    id: 5,
    title: "A Thousand Years",
    artist: "Christina Perri",
    src: "/music/Christina Perri - A Thousand Years.mp3",
    image: null,
    gradient: "linear-gradient(135deg, #fbc2eb 0%, #a18cd1 100%)",
  },
  {
    id: 6,
    title: "Blank Space",
    artist: "Taylor Swift",
    src: "/music/Taylor Swift - Blank Space.mp3",
    image: null,
    gradient: "linear-gradient(135deg, #ff0844 0%, #ffb199 100%)",
  },
  {
    id: 7,
    title: "Love Me Like You Do",
    artist: "Ellie Goulding",
    src: "/music/Ellie Goulding - Love Me Like You Do (Lyrics).mp3",
    image: null,
    gradient: "linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)",
  },
  {
    id: 8,
    title: "Night Changes",
    artist: "One Direction",
    src: "/music/One Direction - Night Changes (Lyrics).mp3",
    image: null,
    gradient: "linear-gradient(135deg, #0c0c3a 0%, #1a1a5e 40%, #6366f1 100%)",
  },
  {
    id: 9,
    title: "Way Back Home",
    artist: "Conor Maynard (Shaun)",
    src: "/music/[Lyrics] Way Back Home - Conor Maynard (Shuan)   Music Full English version.mp3",
    image: null,
    gradient: "linear-gradient(135deg, #a18cd1 0%, #fbc2eb 50%, #667eea 100%)",
  },
  {
    id: 10,
    title: "Theme Music",
    artist: "Portfolio",
    src: "/music/Theme Music.mp3",
    image: null,
    gradient: "linear-gradient(135deg, #667eea 0%, #43e97b 100%)",
  },
];

export default tracks;
